$(function () {
    ...
    $('#btnGuardar').on('click', function (e) {
        ...
        if (Errors.length > 0 || sinRows) {
            ...
            if (sinRows) {
                alert("Debes de agregar por lo menos una cobertura.");
            }
        }
        else {
            ...
            $.ajax({
                type: 'POST',
                url: window.rootUrl + "Plan/RegistraPlan",
    ...
    $("#tblConsulta tbody").on('click', '.delete', function (e) {
        ...
        $(this).closest('tr').find('td').each(function (column, td) {
            ...
            if (itemName == 'scPId') {
                ...
                $.ajax({
                    type: 'POST',
                    url: window.rootUrl + "Plan/EliminaPlan",
    ...
    $("#btnAdd").on('click', function (e) {
        ...
        if (nombre != '') {
            $('#tblPlan').show();
            var contenido = '<tr id="' + cve + '">' + '<td name="scPId">' + cve + '</td>' + '<td name="scPNombre">' + nombre + '</td>' + '<td><div class="form-group text-center"><button class="btn lineAjust deleteR">Borrar</button></div>' + '</tr>';
